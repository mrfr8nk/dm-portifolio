...
const Footer = () => {
  ...
  const links = (footerLinks || []) as Array<{ section: string; label: string; href: string }>;
  const placeholderUrls = new Set(["https://github.com", "https://www.linkedin.com", "mailto:darrell@example.com"]);
  links.forEach((l) => {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-10 mb-16">
          <div className="col-span-2 md:col-span-1">
            <div className="flex items-center gap-2 mb-3">
              <img src={dmLogo} alt="Darrell Mucheri logo" className="w-7 h-7 rounded-md object-cover bg-card border border-border" />
          ...
          <div className="flex gap-2">
            {(socials as unknown as Array<{ id: string; icon_name: string; url: string; platform: string }> | undefined)
              ...
              const Icon = getIcon(s.icon_name);
                return (
                  <a key={s.id} href={s.url} aria-label={`Open ${s.platform} in a new tab`} target="_blank" rel="noopener noreferrer" className="w-8 h-8 rounded-md border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:border-foreground/20 transition-colors focus-ring">
